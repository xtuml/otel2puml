...
def expected_puml_graph_post_loop_insertion() -> PUMLGraph:
    ...
    and_start, and_end = graph.create_operator_node_pair(PUMLOperator.AND)
    graph.add_puml_edge(A, and_start)
    graph.add_puml_edge(and_end, F)
    graph.add_puml_edge(and_start, LOOP1)
    graph.add_puml_edge(and_start, LOOP2)
    graph.add_puml_edge(and_start, LOOP3)
    graph.add_puml_edge(LOOP1, and_end)
    graph.add_puml_edge(LOOP2, and_end)
    graph.add_puml_edge(LOOP3, and_end)
    return graph
...
def expected_loop_1_graph() -> PUMLGraph:
    ...
    graph.add_puml_edge(xor_start, B)
    graph.add_puml_edge(xor_start, C)
    graph.add_puml_edge(B, D)
    graph.add_puml_edge(C, E)
    graph.add_puml_edge(D, xor_end)
    graph.add_puml_edge(E, xor_end)
    return graph
...
def expected_loop_2_graph() -> PUMLGraph:
    ...
    graph.add_puml_edge(G, LOOP)
    graph.add_puml_edge(LOOP, INODE)
    return graph
