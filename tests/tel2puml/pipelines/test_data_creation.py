...
def test_generate_valid_jobs_from_puml_file() -> None:
    ...
    result = list(generate_valid_jobs_from_puml_file(
            input_puml_file
        )
    )
    # assert
    ...
    assert set(
        [event.event_type for event in job.events]
    ) == set(
        ['A', 'B', 'C', 'D', 'E', 'F']
    )
...
def test_generate_event_jsons() -> None:
    ...
    """
    jobs = list(generate_valid_jobs_from_puml_file(
    ...
    assert all(
        set(
            ['eventId', 'eventType', 'timestamp', 'jobId']
        ).issubset(set(event.keys())) for event in result
    )
    assert set(
        [event['eventType'] for event in result]
    ) == set(
        ['A', 'B', 'C', 'D', 'E', 'F']
    )
...
def test_generate_test_data_num_paths_to_template():
    ...
    input_puml_file = "puml_files/ANDFork_ANDFork_a.puml"
    result = list(generate_test_data(
        input_puml_file, num_paths_to_template=10
    ))
    # assert
...
def test_generate_test_data_template_all_paths_true_num_paths_zero():
    ...
    input_puml_file = "puml_files/ANDFork_ANDFork_a.puml"
    result = list(generate_test_data(
        input_puml_file, num_paths_to_template=0
    ))
    # assert
    assert len(result) == 6
    assert all(
...
def test_generate_test_data_template_all_paths_false_large_num_paths():
    ...
    num_templates = 10000
    result = list(generate_test_data(
        input_puml_file, template_all_paths=False, num_paths_to_template=num_templates
    ))
    # assert
    event_types_count = {
        "C": 0,
