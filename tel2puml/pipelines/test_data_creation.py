...
def generate_test_data(
    ...
    event_gen_options: dict | None = None,
    ...
    :param event_gen_options: Options to pass to the event generator. See the test_event_generator documentation for more information.
    :type event_gen_options: `dict` or `None`
    :return: A generator that yields the test data.
    :rtype: `Generator[dict, Any, None]`
    """
    if event_gen_options is None:
        event_gen_options = {}
    if job_gen_options is None:
        job_gen_options = {}
    test_jobs_with_info = puml_file_to_test_events(input_puml_file, **event_gen_options)
    test_job_templates = []
    for test_job_event_list, *_ in test_jobs_with_info[list(test_jobs_with_info.keys())[0]]["ValidSols"][0]:
        ...
        test_job_templates.append(job)
    counter = 0
    if template_all_paths:
        yield from generate_event_jsons(test_job_templates)
